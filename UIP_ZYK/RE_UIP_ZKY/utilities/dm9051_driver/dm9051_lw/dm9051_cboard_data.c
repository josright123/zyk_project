...
//#define printf(fmt, ...) _TASK_DM9051_DEBUGF(TASK_SEMAPHORE_NETCONF_ON, /*SEMA_OFF*/ SEMA_ON, /*"[D]"*/ "", (fmt, ##__VA_ARGS__))
#include "dm9051_cboard_data.h"

#define kkmin(a, b) (a < b) ? a : b

static int printf_space_init(size_t tlen) {
	char cspace[12];
	int n = sprintf(cspace, "rxlen %d", tlen);
	printf(cspace);
	return n;
}
