...
uint8_t DM9051_Read_Reg(uint8_t Reg_Off)
{
    ...
    tx_data[0] = Reg_Off;
    tx_data[1] = 0;
    SPIDRV_MTransferB(SPI_HANDLE, tx_data, result, 2);
    return result[1];
}
...
void DM9051_Write_Reg(uint8_t Reg_Off, uint8_t spi_data)
{
    ...
    tx_data[0] = cmdaddr;
    tx_data[1] = spi_data;
    SPIDRV_MTransmitB(SPI_HANDLE, tx_data, 2);
}
...
void DM9051_Read_Mem(uint8_t *pu8data, uint32_t datalen)
{
  ...
  SPIDRV_MTransferB(SPI_HANDLE, tx_data, pu8data, datalen+1);
  for(i=1; i<datalen+1; i++) {
      pu8data[i-1] = pu8data[i];
  }
}
...
void DM9051_Write_Mem(uint8_t *pu8data, uint32_t datalen)
{
    ...
    for(i=0; i< datalen; i++) {
        tx_data[i+1]=pu8data[i];
    }
    SPIDRV_MTransmitB(SPI_HANDLE, tx_data, datalen+1);
