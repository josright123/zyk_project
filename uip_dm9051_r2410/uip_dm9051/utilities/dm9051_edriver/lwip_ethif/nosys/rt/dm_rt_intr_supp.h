...
#if defined(ETHERNET_INTERRUPT_MODE) //#ifdef DRV_INTR_MODE
...
//	#undef printf
//	#define printf(fmt, ...) TASK_DM9051_DEBUGF("  [ETH.POLLx.INT] ", (fmt, ##__VA_ARGS__))
	printf("[ec %d --> %d]\r\n", nExpireCount_in, nExpireCount);
//	#undef printf
//	#define printf(fmt, ...) TASK_DM9051_DEBUGF("  [[ETH] ] ", (fmt, ##__VA_ARGS__))
}
...
//	#undef printf
//	#define printf(fmt, ...) TASK_DM9051_DEBUGF("[NET] ", (fmt, ##__VA_ARGS__))
}
